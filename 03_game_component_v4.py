...
class Play:
    ...
    def __init__(self, current_bet):
        ...
        control_button_list = [
            [self.button_frame, "Hit Me!", "#FDF4E9", "#981C1E", 0, 0, 10, self.hit_me],
        ...
        for item in control_button_list:
            make_control_button = Button(item[0], text=item[1], fg=item[2], bg=item[3],
                                         command=item[7], font=("Arial", "16", "bold"),
                                         width=item[6])
            make_control_button.grid(row=item[4], column=item[5], padx=5, pady=5)

            control_ref_list.append(make_control_button)

        # retrieve buttons so they can be configured
        self.help_button = control_ref_list[2]
        ...
        self.player_label2 = Label(self.player_frame, text="", bd=0, bg="#042B01")
        self.player_label2.pack(ipady=10, padx=20)
    ...
    def hit_me(self):
        ...
        player.append(card)
        # output card to screen
        player_image2 = self.resize_cards(f'cos_images/{card}.png')
        # create new label to display card
        self.player_label2.config(image=player_image2)
